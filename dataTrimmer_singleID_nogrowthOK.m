...
%        3. determine ratio of negatives to positives
%        4. store ratios from all tracks to reference during removal
%  5. determine which tracks in current movie fall below threshold
%  6. report total number of tracks being removed from current movie
%  7. skip to next movie if nothing is too wiggly
%  8. otherwise, remove structures based on row # (in reverse order)
%  9. save tracks that are too swiggly into reject data matrix
...
for n = 1:length(D)
    ...
    % 8. remove structures based on row # (in reverse order)
    swiggle_counter = 0;
    ...
    % 9. save tracks that are too swiggly into reject data matrix
    rejectD{criteria_counter,n} = swigglyTracks;
