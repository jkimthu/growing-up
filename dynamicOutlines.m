...
%           2. for each image, initialize current image
%                   3. define major axes, centroids, angles, growth rates
%                   4. draw ellipses from image
%                   5. color based on growth rate
%                   6. save and display
%           7. 


% last edit: jen, 2017 Jun 21
...
% 1. isolate ellipse data from movie (stage xy) of interest
dm_currentMovie = dataMatrix(dataMatrix(:,31) == n,:); % col 31 is movie number (n)
...
for img = 1:length(names)
    ...
    % 2. initialize current image
    I=imread(names{img});
    filename = strcat('dynamicOutlines-frame',num2str(img),'.tif');
    ...
    if sum(dm_currentMovie(:,30) == img) == 0
        ...
        % 3. else, isolate data for each image
        dm_currentImage = dm_currentMovie(dm_currentMovie(:,30) == img,:);
        ...
        % 4. for each particle in current image, draw ellipse
        for p = 1:length(majorAxes)
            
            
            [outline1, outline2] = drawEllipse(p,majorAxes, minorAxes, centroid_X, centroid_Y, conversionFactor);
            
            hold on
            plot(outline1,outline2,'lineWidth',2)
            xlim([0 2048]);
            ylim([0 2048]);
        end
        saveas(gcf,filename)
