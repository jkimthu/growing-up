...
%% Part A. division events over time
...
for condition = 1:length(bubbletime)
    ...
    % 5. isolate division events and when they occur
...
%% Part B. divisions as a fraction of total cells over time
...
for condition = 1:length(bubbletime)
    ...
    bins_small = ceil(div_t*binsPerHour);
    bin_counts = accumarray(bins_small,divisions,[],@sum);
    clear divisions div_t bins_small
    
    
    % 7. count number of unique cells per time bin
    trackNum = getGrowthParameter(conditionData_bubbleTrimmed,'trackNum');
    times_h = times_s./3600;
    bins_tracks = ceil(times_h*binsPerHour);
    bin_allcells = accumarray(bins_tracks,trackNum,[],@(x) {x});
    bin_uniquecells = cellfun(@unique,bin_allcells,'UniformOutput',false);
    unique_cells_per_bin = cellfun(@length,bin_uniquecells);
    clear times_s
    
    
    % 8. divide division events by unique cells
    divisions_per_1000cells_per_bin = (bin_counts./unique_cells_per_bin) *1000;
    clear bin_allcells bins_tracks times_h bin_uniquecells
    
    
    % 9. generate time vector for bins
    time_vec = (1:length(bin_counts))*bin;
    time_vec = time_vec/60; % converted from min to hours
    
    
    % 10. plot division events and total cells over time
    palette = {'DodgerBlue','Indigo','GoldenRod','FireBrick'};
    color = rgb(palette(condition));
    
    
    figure(1)
    plot(time_vec,divisions_per_1000cells_per_bin,'Color',color)
    hold on
    xlim([3,maxTime])
    xlabel('Time (h)')
    ylabel('Division events per 1000 cells')
    title(strcat(date))
    
    
    figure(2)
    plot(time_vec,unique_cells_per_bin,'Color',color)
    hold on
    xlim([3,maxTime])
    xlabel('Time (h)')
...
figure(1)
legend('60 min fluc','low','ave','high')

figure(2)
legend('60 min fluc','low','ave','high')


%% Part C. box plot with scatter of growth rate of pre- and post- division in Chigh and Clow
